...
namespace my {
    ...
    template<bool isObject, class T>
    struct destroy_impl {
    ...
    template<class T>
    struct destroy_impl<false, T> {
        static constexpr void destroy(T*) {}
    };

    template<class T>
    constexpr void myDelete(T* ptr) {
        destroy_impl<std::is_object_v<T>, T>::destroy(ptr);
    }

    template<class T, size_t DEFAULT_CAPACITY = 2>
    class Vector {
        T* arr;
        size_t inUse;
        size_t allocated;
    ...
    public:
        ...
        void push_back(const T& val) {
            ...
            new (arr + inUse++) T(val); //calls new placement operator
        }
        ...
        void pop_back() { myDelete(arr + --inUse); }
        ...
        [[nodiscard]] constexpr bool isEmpty() const { return inUse == 0; }
